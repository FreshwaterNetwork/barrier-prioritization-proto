...
function ( declare, PluginBase, FeatureLayer, SimpleLineSymbol, SimpleFillSymbol, SimpleMarkerSymbol, 
			SpatialReference, Geoprocessor, QueryTask, Query, Graphic, InfoTemplate, Color, ContentPane, HorizontalSlider, 
		...
		return declare(PluginBase, {
			toolbarName: "Aquatic Barrier Prioritization", showServiceLayersInLegend: true, allowIdentifyWhenActive: true, rendered: false, resizable: true,
			...
			activate: function () {
               ...
               // $('#legend-container-' + this.mapSide).addClass("hideLegend");
                ...
                // $('#' + this.map + '_layers').on('click', lang.hitch(this, function(e){
                    // console.log("clicked on "+ e.id );
                    // executeQueryTask;
                // }));
			},
			...
			render: function() {			    
                ...
                $('#' + this.appDiv.id + 'toggleResults').on('change', lang.hitch(this, function(evt){             
                        ...
                        if (!ischecked){this.map.graphics.hide();}                        
                })); 
				...
				function displayResult(result, messages){
					...
					dojo.connect(this.map.graphics, "onMouseOver", lang.hitch(this, function(evt) {	
					    ...
					    if (this.keepInfoWindow == "no"|| this.graphicMouseovers >3){			    
    						this.g = evt.graphic;
